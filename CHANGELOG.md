...
- **修复**：中文标点后没有空格导致整段中文被当成一句话（正文超过 180 字会被截断丢弃）→ 按中英文标点分句，分页恢复且不再丢内容；小节要点超过 8 条时第 9 条起静默消失 → 自动续页；`build-deck.mjs` 的 `--content deck.md`（README 示例）此前不读文件、把路径当正文 → 现在路径存在也会读取。
- **修复**：产物 `deck.json` 的 `version` 永远是 0.3.0 → 从 `package.json` 读取（独立复制 skills 目录时回退 0.0.0）；英文 deck 在 PowerPoint 中被标成 zh-CN → `lang` 属性随 `--lang` 下发（正文 / 表格 / 备注页）。
- **稳健性**：三件套（json/html/pptx）改为「全写临时文件 → 逐个 rename」原子提交，中途失败回滚，不再出现「json/html 已更新、pptx 还是旧的」混版；过滤 Windows 保留设备名（CON / NUL / COM1…）；文件名探测由逐候选 `existsSync` 改为一次目录快照。
- 测试 48 → 60 项。
